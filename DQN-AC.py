...
class DQN(nn.Module):
    ...
    def forward(self, x,y):
        ...
        z=torch.tanh(self.dense1(z))
        z=torch.tanh(self.dense2(z))
        z=torch.tanh(self.dense3(z))
...
class DDQNAgent:
    def __init__(self, env, device, epsilon_decay,
        ...
        self.online_critic_network = DQN(self.env, self.learning_rate).to(self.device)
        self.target_critic_network = DQN(self.env, self.learning_rate).to(self.device)
        self.online_actor_network= LSTM_ACT(self.env, self.learning_rate).to(self.device)
        self.target_actor_network= LSTM_ACT(self.env, self.learning_rate).to(self.device)

    def choose_action(self,step, observation,use_actor, greedy = False):
        ...
        # if eps>epsilon:
            ...
            action_pred=self.online_actor_network(observation)
            q_values=self.online_critic_network(observation,action_pred)
            ...
            action=random.choice([-1,0,1])
        return action,epsilon
    def learn(self,batch_size,update_target,train_actor_alone):
        ...
        actions_target = self.target_actor_network(observations_t)

        target_q_values=self.target_critic_network(new_observations_t,actions_target)
        max_target_q_values = target_q_values.max(dim=1, keepdim=True)[0]
        targets = rewards_t + self.discount_rate * (1-dones_t) * max_target_q_values  
        # print(observations_t.shape)
        actions_online= self.online_actor_network(observations_t)

        q_values = self.online_critic_network(observations_t,actions_online)
        # actions_t=actions_t+1
        actions_t=torch.argmax(actions_online,dim=1)
        ...
        output.backward()
        if train_actor_alone:
            optimizer = torch.optim.SGD(params=self.online_actor_network.parameters(), lr=0.001)
        else:

            optimizer = torch.optim.SGD( [{'params': self.online_actor_network.parameters()},
    {'params': self.online_critic_network.parameters()}], lr=0.001)
        optimizer.step()
        if update_target==True:
            self.target_actor_network.load_state_dict(self.online_actor_network.state_dict())
            self.target_critic_network.load_state_dict(self.online_critic_network.state_dict())
...
def training_loop(env,agent, max_episodes, target_ = False, seed=42):
    ...
    for step in range(max_episodes):
        ...
        action, epsilon = agent.choose_action(step, obs,use_actor)
        ...
        update_agent=False
        #Learn
        train_actor_alone=False

        if step%500:
            update_agent=True
        if step<100000:
            train_actor_alone=True
            # print('training actor alone')
        agent.learn(batch_size,update_agent,train_actor_alone)
